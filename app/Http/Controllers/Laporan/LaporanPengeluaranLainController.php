...
class LaporanPengeluaranLainController extends Controller
{
    public function index(Request $request)
    {
        $data = null;
        if ($request->tanggal_awal and $request->tanggal_akhir) {
            $data = PengeluaranLainnya::batal(0)->where([
        ...
        return view('laporan.pengeluaranlain', compact('data'));
    }
    ...
    public function cetak(Request $request, FpdfHtmlService $pdf)
    {
        ...
        $pdf->SetTopMargin(30);
        $pdf->Cell($pdf->GetPageWidth() - 10, 0, 'PENGELUARAN LAIN', 0, 0, 'C');
        ...
        $pdf->Cell(65, 7, 'Nama Pengeluaran', 1, 0, 'C', true);
        ...
        foreach ($data as $item) {
            ...
            $pdf->Cell(65, 7, $item->nama_pengeluaran, 1, 0, 'L', $fill);
        ...
        $pdf->Output('I', 'Laporan Pengeluaran Tanggal ' . $request->tanggal_awal . " sd $request->tanggal_akhir" . '.pdf');
        exit;
    }
}
